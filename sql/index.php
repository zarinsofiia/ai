...
<style>
  ...
  .sql-body { flex: 1; display: flex; flex-direction: column; overflow: hidden; }
  .sql-messages {
    flex: 1; overflow-y: auto; padding: 16px 16px 10px; display: flex; flex-direction: column; gap: 10px;
    scrollbar-width: thin; scrollbar-color: #444 transparent;
  }
  .sql-messages::-webkit-scrollbar { width: 6px; }
  .sql-messages::-webkit-scrollbar-thumb { background-color: #444; border-radius: 3px; }
  .sql-messages::-webkit-scrollbar-track { background: transparent; }
  ...
  .sql-input-container {
    ...
    background-color: #1e1e1e; border-top: 1px solid #333; position: relative;
  }
  .sql-input {
    flex: 1; padding: 14px; border-radius: 10px; border: 1px solid #444;
    font-size: 15px; background-color: #2b2b2b; color: #f1f1f1; outline: none;
  }
  .sql-btn {
    padding: 14px; border: none; border-radius: 10px; background-color: #2a2a2a; color: #f1f1f1;
    font-size: 16px; cursor: pointer; display: flex; align-items: center; justify-content: center;
    transition: background-color .2s;
  }
  .sql-btn:hover { background-color: #3a3a3a; }

  /* Typing dots animation inside the assistant bubble */
  .typing { display: inline-flex; align-items: center; gap: 6px; min-height: 12px; }
  .typing .dot { width: 6px; height: 6px; border-radius: 50%; background: #bbb; opacity: .2; animation: blink 1s infinite ease-in-out; }
  ...
  /* tiny latency badge */
  .latency-badge{
    margin-left:.5rem; padding:.15rem .45rem; border:1px solid #2f2f2f; border-radius:999px;
    font:12px/1.2 system-ui; color:#9ca3af; background:#181818; white-space:nowrap;
  }

  details > summary { list-style: none; cursor: pointer; user-select: none; }
  details > summary::marker, details > summary::-webkit-details-marker { display: none; }
  details > summary { font-size: 13px; color: #9ca3af; }
</style>
</head>
...
<body>
  ...
  ;(() => {
    ...
    async function askMSSQLAI() {
      ...
      let serverLatencyMs = null, summaryPlain = null, sqlText = null, resultObj = null, errorText = null;
    ...
    async function saveLogLocally({ source, question, summary, sqlText, result, errorText, latencyMs }) {
      try {
      ...
      } catch (e) { console.warn('saveLogLocally failed:', e.message); }
    }
    ...
    function getRowCountFromResultJSON(resultJson) {
      const r = safeParse(resultJson); if (!r) return null;
    ...
    inputEl.addEventListener('keydown', (e) => {
      if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); askMSSQLAI(); }
    });
